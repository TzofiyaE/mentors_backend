...
router.post("/", requireAuth, async (req: AuthedRequest, res) => {
  ...
  if (!userDoc.exists || userDoc.data()?.role !== "mentee") {
    res.status(403).json({ error: "Only mentees can send mentorship requests" });
    return;
  }

  const { mentorId, topic, description } = req.body;
  if (!mentorId || !topic) {
    res.status(400).json({ error: "mentorId and topic are required" });
    return;
  }
...
router.patch("/:id", requireAuth, async (req: AuthedRequest, res) => {
  ...
  if (isMentor && status && MENTOR_TRANSITIONS.includes(status)) {
    ...
    res.status(403).json({ error: "Not allowed to perform this transition" });
    return;
  }
